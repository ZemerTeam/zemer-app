...
class FCastDiscoveryHandler : DeviceDiscovererEventHandler {
    ...
    // True while a status viewer (StoryScreen / SavedStatusScreen) is playing a video on the phone
    // itself (the receiver is paused underneath it). Read by the hardware-volume-key routing so those keys adjust the local
