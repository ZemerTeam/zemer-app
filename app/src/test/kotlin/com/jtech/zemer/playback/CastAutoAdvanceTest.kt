...
class CastAutoAdvanceTest {
    ...
    fun `nearEnd is true exactly at the epsilon boundary and past the end`() {
        // duration 180s, idle epsilon 2.0s -> boundary is 178.0s
        assertTrue(CastAutoAdvance.nearEnd(180.0, 178.0, CastAutoAdvance.IDLE_END_EPSILON_SEC))
        assertTrue(CastAutoAdvance.nearEnd(180.0, 181.0, CastAutoAdvance.IDLE_END_EPSILON_SEC))
    }

    @Test
    fun `nearEnd is false before the epsilon boundary`() {
        assertFalse(CastAutoAdvance.nearEnd(180.0, 177.99, CastAutoAdvance.IDLE_END_EPSILON_SEC))
    ...
    fun `nearEnd is false when duration is unknown (zero or negative)`() {
        ...
        assertFalse(CastAutoAdvance.nearEnd(0.0, 0.0, CastAutoAdvance.IDLE_END_EPSILON_SEC))
        assertFalse(CastAutoAdvance.nearEnd(-1.0, 0.0, CastAutoAdvance.STALL_END_EPSILON_SEC))
    }

    @Test
    fun `stall epsilon is wider than idle epsilon`() {
        // At 177.0s of a 180s track: a stall counts as finished, an idle report does not yet.
        assertTrue(CastAutoAdvance.nearEnd(180.0, 177.0, CastAutoAdvance.STALL_END_EPSILON_SEC))
        assertFalse(CastAutoAdvance.nearEnd(180.0, 177.0, CastAutoAdvance.IDLE_END_EPSILON_SEC))
    }
    ...
    fun `idle near end advances when debounced but not within the debounce window`() {
        val now = 1_000_000L
        val nearEnd = CastAutoAdvance.nearEnd(200.0, 199.0, CastAutoAdvance.IDLE_END_EPSILON_SEC)
        assertTrue(nearEnd && CastAutoAdvance.debouncePassed(now, now - 9_000))
        assertFalse(nearEnd && CastAutoAdvance.debouncePassed(now, now - 3_000))
    }
