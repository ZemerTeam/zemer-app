...
- **Generated by `docs/generate.py`** (never hand-edit; regenerated on every push to `main` by `.github/workflows/docs-regenerate.yml`): `reference/kotlin-files.md`, `reference/non-kotlin-files.md`, `reference/resource-index.md`, `build-release.md`, and the inventory tail of `repository-map.md` (from its `### Counts` heading down).
- **Hand-maintained**: everything else - the top of `repository-map.md`, the `app/`, `innertube/` and `ui/` inventories (script-derived from the tree but not regenerated automatically, so re-derive them when their scope changes), and the per-feature docsets, which are prose that cites the source files.
...
| [`app/database.md`](app/database.md) | Room database facts: schema 36 (19 entities, 3 views), fields, primary keys, indices, migrations, DAO annotation counts, and DAO method inventory. |
