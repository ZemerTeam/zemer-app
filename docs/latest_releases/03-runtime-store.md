...
`app/.../latestreleases/LatestReleasesStore.kt`. A process-wide `object` (`:67`) that owns the
feed at runtime: an in-memory copy, a disk cache for instant display, and a conditional network
refresh. Modelled on the cipher `PlayerConfigStore` (its KDoc says so, `:49-52`).
...
DB-backed sections (`:54-57`).

### `refresh(): List<LatestRelease>` (`:128-156`) — the network path
...
3. `repeat(MAX_ATTEMPTS)`:
   ...
   - **`NotModified`** (304) -> re-stamp the meta's last-fetch time to now (`writeMeta`), keep the
     cache, return `cachedReleases()` (`:141-145`).
   - **`Failure`** -> if not the last attempt, `delay(retryDelayMs)` then loop (`:146-149`).
...
### `applyFetched(body, etag)` (`:158-174`)
...
1. Parse `body` into `LatestReleasesFeed`. **On a parse exception, log and return the previous
   ...
   (`:159-164`; test `an unparseable body keeps the previous releases`, `:106-115`).
...
3. Persist: write the body to the cache file atomically and stamp the meta. **Persist failures
   are caught** — the feed stays in memory even if disk write fails (`:167-172`).
4. Return `feed.releases`.

### `httpFetch(etag)` (`:176-186`) — the default `fetcher`

A Ktor `get(FEED_URL)`:
- Sends `If-None-Match: <etag>` when a non-empty ETag is stored (`:178`).
...
### `readValidDiskCache()` (`:189-205`) — the staleness gate

1. Read the meta's last-fetch timestamp.
2. **If the meta is missing, or `now - lastFetch > MAX_STALE_MS` (3 days), drop the cache**
   (`clearDiskCache()`) and return null (`:191-196`). This is why a server that has been down
...
### Atomic writes (`writeAtomic`, `:241-248`)
