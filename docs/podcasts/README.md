...
through the same pipeline as a song). The app<->server threads live in
`handoff-docs/zemer-app-podcasts-request.md` and its follow-ups.
...
- **Browse** (`WhitelistedPodcastsScreen`): the channel grid from the allow-set, with an instant
  ...
  filter, never a networked results page.
...
- **Home Podcasts tab**: genre strip (`HomePodcastGenresRow` over the shared `HomeGenreChipsStrip`,
  hidden by the same `ShowHomeGenresKey` as the music strip; the full catalog is `PodcastGenresScreen`,
  server-owned `kind` sections via `podcastGenreSections` + the shared `GenreCardGrid`), ranked rows
  from `/podcast-home-rows` (fail-soft, live-only),
...
- **Episode resume**: `playback/EpisodePositionTracker` (driven by `MusicService`) persists
  `lastPositionMs` on every exit path and seeks on load; the pure `EpisodeResume` decides edges
  (no resume within `RESUME_EDGE_MS` of the start, restart within `COMPLETION_EDGE_MS` of the end).
...
`/podcasts/new-episodes`, and podcast groups in `/search`; the allow-set comes from the content
mirror (above). All are server-first with the offline-snapshot fallback except the live-only
`/podcast-home-rows` and `/podcast-channel` offset paging (`podcastChannelEpisodes`). Contracts: the
`zemer-app-podcast*` handoff docs.
