...
— or a single stall within 15s of first reaching READY — drop DIRECTLY to the highest rung the
player's measured bandwidth sustains with 1.3× headroom (one decisive jump, not a stall per
intermediate rung). Seek-caused buffering is exempt via a timestamp grace window; a swap's own prepare
never counts; LOCAL/audio never count; AUTO never downgrades. The shared LoadControl is left at
media3's rebuffer default (widening it would regress audio/RELAY recovery).
